...
Query.prototype.qt = function(type){
  var self = this;
  var parameter = 'qt=' + type;
  this.parameters.push(parameter);
  return self;
}
 ...
 * @param {String} options.fl - The fields to use for similarity. NOTE: if possible, these should have a stored TermVector
 * @param {String} options.count - The number of similar documents to return for each result.
 * @param {String} [options.mintf] - Minimum Term Frequency - the frequency below which terms will be ignored in the source doc.
 * @param {String} [options.mindf] - Minimum Document Frequency - the frequency at which words will be ignored which do not occur in at least this many docs.
 ...
 * @param {Boolean} [options.maxntp] - maximum number of tokens to parse in each example doc field that is not stored with TermVector support.
 * @param {String} [options.boost] - [true/false] set if the query will be boosted by the interesting term relevance.
 * @param {String} [options.qf] - Query fields and their boosts using the same format as that used in DisMaxQParserPlugin. These fields must also be specified in mlt.fl.
 */
...
Query.prototype.mlt = function(options){
  ...
  if(options.count){
    this.parameters.push('mlt.count=' + encodeURIComponent(options.count))
  }
  if(options.mintf){
    this.parameters.push('mlt.mintf=' + encodeURIComponent(options.mintf))
  }
  if(options.mindf){
    this.parameters.push('mlt.mindf=' + encodeURIComponent(options.mindf));
  }
  if(options.minwl){
    this.parameters.push('mlt.minwl=' + encodeURIComponent(options.minwl))
  }
  if(options.maxwl){
    this.parameters.push('mlt.maxwl=' + encodeURIComponent(options.maxwl))
  }
  if(options.maxqt){
    this.parameters.push('mlt.maxqt=' + encodeURIComponent(options.maxqt))
  }
  if(options.maxntp){
    this.parameters.push('mlt.maxntp=' + encodeURIComponent(options.maxntp));
  }
  if(options.boost){
    this.parameters.push('mlt.boost=' + encodeURIComponent(options.boost));
  }
  if(options.qf){
    this.parameters.push('mlt.qf=' + encodeURIComponent(options.qf));
  }
  return self;
}
